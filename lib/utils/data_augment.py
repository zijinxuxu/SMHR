...
def random_crop(image, gts, igs, crop_size, limit=8):
    ...
    if crop_w > img_width:
        pave_x = int(np.random.randint(0, crop_w - img_width + 1))
        cropped_image = np.ones((crop_h, crop_w, 3), dtype=image.dtype) * np.mean(image, dtype=int)
        if len(gts)>0:
        ...
        crop_y1 -= diff_y
        cropped_image[:, pave_x:pave_x+img_width] = np.copy(image[crop_y1:crop_y1 + crop_h, :])
...
def random_pave(image, gts, igs, pave_size, limit=8):
    ...
    if pave_w < img_width:
        # for crowdhuman dataset, since varied aspect ratio here
        ...
        if len(gts) > 0:
            ...
            sel_center_x = int(np.random.randint(0, img_width - pave_w + 1) + pave_w * 0.5)

        crop_x1 = max(sel_center_x - int(pave_w * 0.5), int(0))
        diff_x = max(crop_x1 + pave_w - img_width, int(0))
        crop_x1 -= diff_x
        paved_image[pave_y:pave_y + img_height, :] = np.copy(image[:, crop_x1:crop_x1 + pave_w])

        if len(igs) > 0:
            igs[0, 0:4:2] -= crop_x1
            igs[:, 1:4:2] += pave_y
            igs[:, 0:4:2] = np.clip(igs[:, 0:4:2], 0, pave_w)
            igs[:, 1:4:2] = np.clip(igs[:, 1:4:2], 0, pave_h)
            keep_inds = ((igs[:, 2] - igs[:, 0]) >= 8) & \
                        ((igs[:, 3] - igs[:, 1]) >= 8)
            igs = igs[keep_inds]
        if len(gts) > 0:
            ...
            gts[:, 0:4:2] -= crop_x1
            gts[:, 1:4:2] += pave_y
            gts[:, 0:4:2] = np.clip(gts[:, 0:4:2], 0, pave_w)
            gts[:, 1:4:2] = np.clip(gts[:, 1:4:2], 0, pave_h)
...
def augment(img_data, c):
    ...
    img = cv2.imread(os.path.join(c.pre_fix, img_data_aug['filepath']))
