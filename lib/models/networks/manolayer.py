...
class ManoLayer(Module):
    def __init__(self, manoPath, center_idx=9, use_pca=False, new_skel=False):
    ...
    def axis2Rmat(self, axis):
        # axis: bs x 45
        rotation_mat = rodrigues_batch(axis.reshape(-1, 3))
    ...
    def forward(self, root_rotation, pose, shape, trans=None, scale=None, side='left'):
        # input
        ...
        if self.use_pca:
            ...
            # change to rotation matrix due to H2O dataset.
            root_rotation = self.myaxis2Rmat(root_rotation)
            pose = self.myaxis2Rmat(pose)            
            rotation_mat = pose
        ...
        if side == 'left':
            jList = j_withoutTips + [v_output[:, 745], v_output[:, 317], v_output[:, 445], v_output[:, 556], v_output[:, 673]]
        else:
